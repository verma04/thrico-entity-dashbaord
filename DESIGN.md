...
Focus on ONE component at a time. Reference its YAML key ({component.feature-card-pink}, {component.pricing-tier-card-featured}).
Pick the right brand-color card for the feature: pink for outbound/sequencer, teal for enterprise/featured, lavender for AI-agent products, peach for general SaaS warmth, ochre for community / experts.
Variants of an existing component (-active, -disabled) live as separate entries.
