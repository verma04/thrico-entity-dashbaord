...
export default function CommunityAuditLog() {
  ...
  const handleExportLogs = () => {
    ...
    const csv = buildCsv(logs, [
      ...
      { header: "Module", getValue: (l: any) => l.module || "COMMUNITIES" },
      {
        header: "Admin",
        getValue: (l: any) =>
          l.admin
    ...
    downloadCsv(csv, `community-audit-${moment().format("YYYY-MM-DD")}`);
        ...
        {Object.entries(parsedState).map(([key, value]) => {
          ...
          return (
            <div
              key={key}
              className={cn(
                "flex flex-col gap-1 p-3 rounded-xl border shadow-2xs",
                isRed
  ...
  const columns = [
    {
      ...
      cell: (log: any) => (
        <div className="flex flex-col">
          ...
          <span className="text-[10px] text-muted-foreground ml-5 tabular-nums font-medium">
    ...
    },
    {
      ...
      cell: (log: any) => (
        <div className="flex items-center gap-3">
          <div className="h-8 w-8 rounded-lg bg-muted/60 border border-border/60 flex items-center justify-center shrink-0">
          ...
          <div className="flex flex-col min-w-0">
            ...
            <div className="flex items-center gap-2 mt-0.5">
              <span className="inline-flex h-4 items-center px-1.5 rounded text-[9px] font-semibold uppercase tracking-widest bg-muted text-muted-foreground border border-border/40">
                {log.module || "COMMUNITIES"}
              </span>
            </div>
          </div>
        </div>
            ...
            {hasAdmin && log?.admin?.id ? (
              <UserProfileHoverCard user={log.admin}>
                <div className="flex items-center gap-2.5 cursor-pointer group">
                  <div className="h-7 w-7 rounded-lg bg-muted border border-border/60 flex items-center justify-center shrink-0 shadow-2xs group-hover:ring-2 group-hover:ring-primary/20 transition-all">
                    <User className="h-3.5 w-3.5 text-muted-foreground" />
                  </div>
                  <div className="flex flex-col min-w-0">
                    <span className="text-xs font-semibold text-foreground group-hover:text-primary transition-colors leading-none truncate">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-card border border-border/60 rounded-xl p-4 shadow-2xs">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            Cryptographically logged modifications, permissions adjustments, and operational events for this {singularName.toLowerCase()}.
          </p>
        </div>
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
                  {[
                    ...
                    {
                      label: "Module",
                      val: logDetailsData.auditLogById.module || "COMMUNITIES",
                      icon: Globe,
                    },
                    {
                      label: "Time",
