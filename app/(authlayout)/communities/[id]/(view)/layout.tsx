...
function CommunitiesLayout({
  ...
  const currentTab =
    pathname === basePath || pathname === `${basePath}/`
      ? "discussion"
      : pathname?.replace(`${basePath}/`, "") || "discussion";
        ...
        <div className="sticky top-0 z-40 bg-background/80 backdrop-blur-xl border-b border-border/60">
          <div className="max-w-7xl mx-auto px-6">
            ...
            <div className="overflow-x-auto scrollbar-hide -mx-6 px-6 sm:mx-0 sm:px-0">
              <nav className="flex items-center gap-1 -mb-px w-max sm:w-full">
                {tabItems.map((tab) => {
                    ...
                    <button
                      ...
                      onClick={() => {
                        if (tab.key === "discussion") {
                          router.push(`/communities/${id}/discussion`);
                        } else {
                          router.push(`/communities/${id}/${tab.key}`);
                        }
                      }}
                      className={cn(
                        "relative flex items-center gap-2 px-4 py-2.5 text-sm font-medium transition-colors rounded-t-lg shrink-0",
                        isActive
                          ? "text-foreground"
