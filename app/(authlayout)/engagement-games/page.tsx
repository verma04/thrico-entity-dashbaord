...
function TodayCard({
  title, icon: Icon, iconBg, iconColor, plays, tcBurned, tcRewarded, href, loading,
}: {
  title: string; icon: any; iconBg: string; iconColor: string;
  plays: number; tcBurned: number; tcRewarded: number; href: string; loading: boolean;
}) {
          ...
          <>
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                { label: "TC Burned", value: tcBurned.toLocaleString(), mono: true },
                { label: "TC Rewarded", value: tcRewarded.toLocaleString(), mono: true },
              ].map((row) => (
                <div key={row.label} className="p-3 rounded-lg bg-muted/30 text-center">
                  <p className={cn("text-base font-bold text-foreground", row.mono && "font-mono")}>{row.value}</p>
            ...
            <div className={cn(
              ...
              isHealthy ? "bg-emerald-50 border-emerald-200 text-emerald-700" : "bg-rose-50 border-rose-200 text-rose-700"
            )}>
              <span>Margin</span>
  ...
  return (
    <EcosystemWrapper>
      ...
      <EcosystemContainer className="p-6 space-y-6">
        ...
        {/* Today's Breakdown */}
        <div>
          <h2 className="text-sm font-semibold text-foreground mb-3">Today's Activity</h2>
        ...
        {/* Economy Summary */}
        <div>
          <h2 className="text-sm font-semibold text-foreground mb-3">Economy Health</h2>
          <div className="rounded-xl border border-border bg-card p-5 space-y-4">
            {[
              ...
              const margin = row.burned > 0 ? ((row.burned - row.rewarded) / row.burned) * 100 : 0;
              const isHealthy = margin >= 20 && margin <= 40;
              return (
                <div key={row.label} className="space-y-1.5">
                  <div className="flex items-center justify-between text-xs">
                    <span className="font-medium text-foreground">{row.label}</span>
                    <span className={cn("font-bold font-mono", isHealthy ? "text-emerald-600" : loading ? "text-muted-foreground" : "text-rose-600")}>
            ...
            <p className="text-xs text-muted-foreground pt-1">Target margin: 20%–40% of TC burned per day</p>
          </div>
        </div>
      </EcosystemContainer>
    </EcosystemWrapper>
