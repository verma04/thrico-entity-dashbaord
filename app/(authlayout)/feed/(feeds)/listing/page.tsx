...
import { useListingFeed } from "@/graphql/actions/feed";
  ...
  const loadMoreData = async () => {
    ...
    try {
      const { data: fetchMoreResult } = await fetchMore({
        variables: {
          input: {
            offset: data?.getListingFeed?.length,
            limit: 10,
          },
        },
        updateQuery(prev, { fetchMoreResult }) {
          if (!fetchMoreResult || fetchMoreResult?.getListingFeed?.length === 0) {
            setHasMore(false);
            return prev;
          } else {
            return Object.assign({}, prev, {
              getListingFeed: [...prev.getListingFeed, ...fetchMoreResult.getListingFeed],
            });
          }
        },
      });
  ...
  return (
    <div className="bg-transparent">
      <div className="px-4 py-6">
        <InfiniteScroll
          dataLength={data?.getListingFeed?.length || 0}
          next={loadMoreData}
          hasMore={hasMore}
          loader={
            isFetchingMore ? (
        ...
        >
          <div className="max-w-2xl mx-auto space-y-6 pb-20">
            {data?.getListingFeed?.map((item: any) => (
              <Feed key={item.id} feed={item} />
            ))}
            {(!data?.getListingFeed || data.getListingFeed.length === 0) && !loading && (
              <div className="text-center py-20 bg-zinc-50 border border-zinc-100 rounded-[32px]">
                 <p className="text-zinc-400 font-bold uppercase tracking-widest text-[11px]">No Listings Found</p>
              </div>
            )}
          </div>
        </InfiniteScroll>
