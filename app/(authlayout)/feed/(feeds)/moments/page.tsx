...
export default function MomentsFeed() {
  ...
  const loadMoreData = async () => {
    ...
    try {
      await fetchMore({
        variables: {
          input: {
            offset: data?.getMomentsFeed?.length || 0,
            limit: 10,
          },
        },
  ...
  const feeds = data?.getMomentsFeed || [];
    ...
    <div className="w-full max-w-2xl mx-auto pb-16">
      {loading && feeds.length === 0 ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 px-6 text-center rounded-2xl border border-dashed border-border/80 bg-card/50">
          <div className="h-14 w-14 rounded-2xl bg-purple-500/10 text-purple-600 dark:text-purple-400 flex items-center justify-center mb-4 shadow-xs">
          ...
          endMessage={
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-xs font-semibold text-muted-foreground">
                You've reached the end of moments
              </p>
            </div>
          }
        >
