...
export default function GamificationOverview() {
  ...
  const kpis = [
    {
      title: "Engaged Users",
      value: statsLoading ? "—" : (gamificationStats?.totalUsers?.toLocaleString() ?? "0"),
      icon: Users,
      color: "text-zinc-900",
      bg: "bg-zinc-100",
    },
    {
      ...
      value: statsLoading ? "—" : (gamificationStats?.totalPointsAwarded?.toLocaleString() ?? "0"),
      icon: Zap,
    ...
    {
      title: "Badges Earned",
      value: statsLoading ? "—" : (gamificationStats?.totalBadgesEarned?.toLocaleString() ?? "0"),
      icon: Award,
    ...
    {
      title: "Active Rules",
      value: statsLoading ? "—" : (gamificationStats?.activePointRules?.toString() ?? "0"),
      icon: Coins,
        ...
        <div className="flex items-center justify-between w-full">
          <div className="flex items-center gap-2 px-1">
            ...
            <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest italic">
              {settings.isEnabled ? "Incentive Protocol Online" : "System Paused"}
            </span>
          </div>
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            >
              <div className="space-y-1 mt-4 overflow-hidden rounded-lg border border-zinc-100">
                {[
                  { label: "Engine Status", value: settings.isEnabled ? "ACTIVE" : "PAUSED", color: settings.isEnabled ? "text-emerald-600" : "text-rose-500" },
                  { label: "Login Rewards", value: reloginConfig.isEnabled ? "ENABLED" : "DISABLED", color: reloginConfig.isEnabled ? "text-indigo-600" : "text-zinc-500" },
                  { label: "Daily Cap", value: settings.dailyPointsCap ? `${settings.dailyPointsCap} pt` : "UNLIMITED", color: "text-zinc-900" },
                  { label: "Point Decay", value: settings.pointDecayEnabled ? "ON" : "OFF", color: settings.pointDecayEnabled ? "text-rose-600" : "text-zinc-500" },
                ].map((row, i) => (
                  <div key={i} className="flex items-center justify-between px-4 py-3 bg-zinc-50/50">
                    <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest leading-none">
                      {row.label}
                    </span>
                    <span className={cn("text-[10px] font-bold tracking-widest leading-none", row.color)}>
                      {row.value}
                    </span>
                  </div>
                ))}
              ...
              <div className="mt-8 space-y-3">
                {[
                  { label: "Leaderboard", icon: Trophy, href: "/gamification/leaderboard" },
                  { label: "Activity Log", icon: History, href: "/gamification/activity-log" },
                ].map((link, i) => (
                  <Link key={i} href={link.href} className="flex items-center justify-between p-3.5 rounded-lg border border-zinc-100 bg-white hover:bg-zinc-50 transition-all group">
                    <div className="flex items-center gap-3">
