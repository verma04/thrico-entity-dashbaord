...
import {
  GET_REWARDS_AUTOMATION_RULE,
  GET_REWARDS_AUTOMATION_RULES,
  UPDATE_REWARDS_AUTOMATION_RULE,
...
const EditRewardsAutomationRulePage = () => {
  ...
  const [updateRule, { loading: updating }] = useMutation(
    UPDATE_REWARDS_AUTOMATION_RULE,
    {
      ...
      onCompleted: () => {
        toast.success("Rewards automation rule updated successfully!");
        router.push("/gamification/rewards/automation");
      },
      onError: (err: any) => {
        toast.error(err.message || "Failed to update rewards automation rule");
      },
    }
  );

  const handleSave = async (input: UpdateRewardsAutomationRuleInput) => {
    await updateRule({
      variables: {
        id: ruleId,
        input,
  ...
  const rule = data?.getRewardsAutomationRule;
      ...
      <div className="fixed inset-0 z-50 bg-background w-screen h-screen flex flex-col items-center justify-center gap-4">
        ...
        <div className="text-center space-y-1">
          <h3 className="text-sm font-bold text-foreground">
            Loading Rewards Studio
          </h3>
          <p className="text-xs text-muted-foreground">
            Fetching rule details & canvas layout…
          </p>
        </div>
      </div>
    );
