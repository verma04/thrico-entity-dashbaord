...
export default function EngagementDashboardPage() {
  ...
  const { data, loading, refetch } = useGetSpinScratchStats(
    timeRange,
    formattedDateRange,
  );
  ...
  const kpis = [
    {
      ...
      trendLabel: `${stats?.spinStatsToday?.plays ?? stats?.spinStats?.plays ?? 0} in range`,
      icon: Dices,
    ...
    },
    {
      ...
      trendLabel: `${stats?.scratchStatsToday?.plays ?? stats?.scratchStats?.plays ?? 0} in range`,
    ...
    },
    {
      ...
      trendLabel: `${stats?.matchWinStatsToday?.plays ?? stats?.matchWinStats?.plays ?? 0} in range`,
      icon: Trophy,
    ...
    {
      title: `Total ${currencyName} Burned`,
    ...
    {
      title: `Total ${currencyName} Rewarded`,
  ...
  return (
    <EcosystemWrapper>
      <EcosystemHeader
        title="Member Games"
        badgeText={gamesCenterModuleName || "Games"}
        description="Overall interactive member games insights and performance"
        icon={Gamepad2}
        breadcrumbs={[
        ...
        <section className="space-y-4">
          ...
          <div className="rounded-[20px] border border-transparent bg-muted/30 p-5 space-y-4">
            ...
            <p className="text-xs text-muted-foreground pt-1">
              Target margin: 20%–40% of {currencyName} burned per day
            </p>
          </div>
        </section>
      </EcosystemContainer>
