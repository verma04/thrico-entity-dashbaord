...
import { Settings, ScrollText, MessageCircleQuestion, Tag } from "lucide-react";
...
function OffersSettingsLayout({ children }: { children: React.ReactNode }) {
  ...
  const handleTabChange = (value: string) => {
    const basePath = "/offers/settings";
    switch (value) {
      case "settings":
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-6 border-b">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl font-bold">Offers Settings</h1>
          </div>
          <p className="text-muted-foreground">
            Configure offers module settings, terms, and FAQs
          </p>
        </div>
      </div>
