...
export default function PollResultsPage() {
  ...
  const { data: pollData, loading: pollLoading } = getPollByIdForUser({
    variables: {
      input: {
        pollId: id,
      },
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="flex-1 bg-card border border-border rounded-xl p-4 shadow-sm">
          {loading ? (
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full space-y-6">
              <TabsList className="h-9 bg-muted/40 border border-border rounded-lg p-0.5 gap-0.5 inline-flex">
                <TabsTrigger 
                  value="overview" 
                  className="h-8 px-4 rounded-md text-xs font-medium gap-1.5 data-[state=active]:bg-background data-[state=active]:shadow-sm data-[state=active]:text-foreground text-muted-foreground transition-all duration-200"
                >
      ...
      <div className="hidden xl:flex flex-col bg-slate-50/50 dark:bg-zinc-950/50 rounded-xl border border-border p-8 relative items-center">
