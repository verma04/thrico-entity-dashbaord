  ...
  return (
    <EcosystemWrapper anonymized-1="rewards-dashboard">
      <EcosystemHeader
        ...
        badgeText="Reward Economy"
        description="Monitor community fulfillment velocity, inventory health, and currency consumption protocols across the global registry node."
        icon={Trophy}
      />
      ...
      <EcosystemActionBar shadow="none">
        <div className="flex items-center justify-between w-full">
           <div className="flex items-center gap-6">
              <EcosystemStatusIndicator status="active" label="Economic Stream: Synchronized" />
              <div className="h-4 w-px bg-slate-200" />
              <div className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest italic">
                 <ShieldCheck className="h-3.5 w-3.5 text-emerald-500" />
                 <span>Verified Economy Node</span>
              </div>
           </div>
      ...
      <EcosystemContainer className="space-y-12 p-8 lg:p-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
           ...
           <div className="lg:col-span-8">
              <EcosystemCard 
                title="Emission Velocity" 
                description="Temporal currency consumption cycles" 
                icon={TrendingUp}
                decorationIcon={Zap}
              >
           ...
           <div className="lg:col-span-4">
              <EcosystemCard 
                title="Economic Feed" 
                description="Recent redemption events" 
                icon={Activity}
              ...
              >
                    ...
                    {redemptionsLoading ? (
                      ...
                      redemptions.map((act: any, i: number) => (
                        <div key={i} className="flex items-center gap-4 p-4 rounded-2xl bg-white border border-slate-100 shadow-sm transition-all hover:shadow-md hover:translate-x-1 group/item">
                          <div className="h-10 w-10 rounded-xl bg-slate-50 flex items-center justify-center font-black text-slate-400 group-hover/item:bg-indigo-600 group-hover/item:text-white transition-colors">
                          ...
                          <div className="flex-1 overflow-hidden">
                            ...
                            <p className="text-[10px] font-bold text-slate-400 truncate uppercase tracking-tight italic">
                              {act.reward?.title}
                            </p>
                          </div>
                          <div className="text-right">
                             <p className="text-[10px] font-black text-indigo-500 whitespace-nowrap">
                 ...
                 <div className="mt-8 pt-8 border-t border-slate-50">
                    <Link href="/rewards/vouchers/redemptions">
                       <Button variant="outline" className="w-full h-12 rounded-xl border-slate-200 font-black text-[10px] uppercase tracking-widest text-slate-600 gap-3 hover:bg-slate-50 transition-all shadow-sm">
                          Global Ledger
        ...
        <div className="space-y-8">
           <div className="flex items-center gap-3 px-1">
              ...
              <div>
                 <h2 className="text-xl font-black text-slate-900 tracking-tight italic uppercase">Foundational Shortcuts</h2>
                 <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.3em] leading-none mt-1">Registry governance & inventory control</p>
              </div>
           </div>

           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
              {[
                { title: "Manage Coupons", desc: "Instantiation & logic configuration", icon: Ticket, link: "/rewards/vouchers/coupons", color: "text-blue-600", bg: "bg-blue-50" },
                { title: "Inventory Audit", desc: "Upload & verify voucher pools", icon: Package, link: "/rewards/vouchers/inventory", color: "text-emerald-600", bg: "bg-emerald-50" },
                { title: "Audit Ledger", desc: "Temporal emission history", icon: History, link: "/rewards/vouchers/redemptions", color: "text-indigo-600", bg: "bg-indigo-50" },
                { title: "Fraud Protocol", desc: "Abuse prevention & invariants", icon: ShieldCheck, link: "/rewards/fraud", color: "text-rose-600", bg: "bg-rose-50" }
              ].map((item, i) => (
                <Link key={i} href={item.link}>
                  <div className="p-8 rounded-[2.5rem] bg-white border border-slate-100 shadow-xl shadow-slate-200/50 group hover:shadow-2xl hover:translate-y-[-8px] transition-all duration-500 relative overflow-hidden">
                    <div className={cn("inline-flex p-4 rounded-2xl mb-6 transition-all duration-500 group-hover:scale-110", item.bg)}>
                       <item.icon className={cn("h-6 w-6", item.color)} />
                    </div>
                    <h3 className="text-lg font-black text-slate-900 tracking-tight italic uppercase mb-2">{item.title}</h3>
                    <p className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest leading-relaxed">{item.desc}</p>
