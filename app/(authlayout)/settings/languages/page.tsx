      ...
      <div className="rounded-xl border border-slate-200/80 bg-white shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
           <div className="flex items-center gap-3">
             ...
             <div>
                <h3 className="text-[14px] font-semibold text-slate-900 tracking-tight">System Language</h3>
                <p className="text-[11px] text-slate-500 mt-0.5">Select the default locale for the admin dashboard.</p>
             </div>
           </div>
        </div>

        <div className="p-5">
           <p className="text-[12px] text-slate-500 mb-6 bg-slate-50 border border-slate-100 p-3.5 rounded-lg max-w-3xl leading-relaxed font-medium">
             ...
             {supportedLanguages.map((lang) => (
                <div
                  ...
                  className={`flex items-start justify-between space-x-4 rounded-xl border p-4 transition-all duration-200 ${
                    lang.active 
                      ? "bg-emerald-50/30 border-emerald-200 shadow-sm shadow-emerald-500/5 ring-1 ring-emerald-500/10" 
                      : "bg-slate-50/50 border-slate-100 opacity-60 grayscale"
                  }`}
                >
                  <div className="space-y-1">
                    <p className={`text-[13px] font-semibold leading-none flex items-center gap-2 ${lang.active ? "text-emerald-900" : "text-slate-600"}`}>
                      {lang.name}
                    </p>
                    <p className="text-[10px] font-mono text-slate-400 font-semibold">{lang.code}</p>
                  </div>
                  {lang.active ? (
                    ...
                    <span className="text-[9px] font-semibold tracking-widest uppercase bg-slate-200 text-slate-500 px-2 py-1 rounded shadow-inner">
                      Soon
                    </span>
                  )}
                </div>
