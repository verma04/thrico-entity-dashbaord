...
import {
  ...
  useAddWallOfFameCategory,
  useUpdateWallOfFameCategory,
  useDeleteWallOfFameCategory,
      ...
      <DialogContent className="max-w-md rounded-2xl border-slate-200">
        <DialogHeader>
          <DialogTitle className="font-bold text-slate-800">
            {editingCategory ? "Edit Category" : "Add Wall of Fame Category"}
          </DialogTitle>
        ...
        <div className="space-y-4 py-2">
          <div className="space-y-2">
            ...
            <Input
              id="cat-title"
              placeholder="e.g., Innovation, Leadership, Community"
              value={title}
          ...
          >
            ...
            <CardContent className="p-6">
              ...
              <p className="text-sm text-slate-600 font-medium mt-4 line-clamp-2 leading-relaxed">
                Wall of Fame distinction classification node
              </p>
            </CardContent>
          </Card>
        );
...
function WallOfFameCategoriesPage() {
  ...
  const [createCategory, { loading: creating }] = useAddWallOfFameCategory({
    onCompleted: () => {
      notify.success("Category created successfully");
      setIsDialogOpen(false);
      refetch();
    },
    onError: (error) => notify.error(error.message || "Failed to create category"),
  });

  const [updateCategory, { loading: updating }] = useUpdateWallOfFameCategory({
    onCompleted: () => {
    ...
    onError: (error) => notify.error(error.message || "Failed to update category"),
  });

  const [deleteCategory, { loading: deleting }] = useDeleteWallOfFameCategory({
    onCompleted: () => {
      notify.success("Category deleted successfully");
      setCategoryToDelete(null);
      refetch();
    },
    onError: (error) => notify.error(error.message || "Failed to delete category"),
  });

  const handleSave = async (values: { title: string }) => {
    if (editingCategory) {
      await updateCategory({
        variables: { updateWallOfFameCategoryId: editingCategory.id, input: { title: values.title } },
      });
    } else {
  ...
  const handleDelete = async () => {
    ...
    await deleteCategory({
      variables: { deleteWallOfFameCategoryId: categoryToDelete.id },
    });
  };

  const categories = data?.getWallOfFameCategories || [];
  const filteredCategories = categories.filter((c: any) =>
    c.title.toLowerCase().includes(search.toLowerCase()),
  );

  return (
    <EcosystemWrapper anonymized-1="wall-of-fame-categories">
      <EcosystemHeader
        title="Wall of Fame Categories"
        badgeText="Organization"
        description="Organize wall of fame entries by category for better discovery and recognition."
        icon={FolderTree}
        actions={
          <Button
