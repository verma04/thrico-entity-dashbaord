...
import { ScrollText, MessageCircleQuestion, Trophy } from "lucide-react";
...
function WallOfFameSettingsLayout({ children }: { children: React.ReactNode }) {
  ...
  const getActiveTab = () => {
    ...
    return "terms"; // Default to terms since no settings page
  };

  const activeTab = getActiveTab();

  const handleTabChange = (value: string) => {
    const basePath = "/wall-of-fame/settings";
    switch (value) {
      case "terms":
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-6 border-b">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl font-bold">Wall of Fame Settings</h1>
          </div>
          <p className="text-muted-foreground">
            Configure wall of fame module terms and FAQs
          </p>
        </div>
      </div>
