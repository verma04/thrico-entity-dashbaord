...
type SelectedNodeInfo =
  | { type: "user"; data: InterestGraphUser; connectedInterests: string[] }
  | {
      type: "interest";
      data: InterestGraphInterest;
      connectedUsers: InterestGraphUser[];
    };
    ...
    return (
      <div className="absolute top-4 left-4 z-20 w-72 rounded-xl bg-white border border-slate-200 shadow-lg overflow-hidden animate-in slide-in-from-left-2 fade-in duration-200">
        ...
        <div className="px-4 pt-2 pb-4 space-y-4">
          ...
          {info.connectedInterests.length > 0 && (
            <div>
              <p className="text-[10px] font-medium text-slate-400 uppercase tracking-wider mb-2">
                Interests ({info.connectedInterests.length})
              </p>
              <div className="flex flex-wrap gap-1.5">
                {info.connectedInterests.map((interest) => (
                  <span
                    key={interest}
    ...
    <div className="absolute top-4 left-4 z-20 w-72 rounded-xl bg-white border border-slate-200 shadow-lg overflow-hidden animate-in slide-in-from-left-2 fade-in duration-200">
      ...
      <div className="px-4 py-4 space-y-4">
        ...
        {info.connectedUsers.length > 0 && (
          <div>
            <p className="text-[10px] font-medium text-slate-400 uppercase tracking-wider mb-2">
              Interested Users ({info.connectedUsers.length})
            </p>
...
export function InterestsGraphView() {
  ...
  const elements = useMemo(() => {
    ...
    const interestNodes = new Map<string, { count: number; data: any }>();
    const graphEdges: any[] = [];

    edges.forEach((edge) => {
      const userId = `user-${edge.user.id}`;
      const interestId = `interest-${edge.interest.id}`;
      ...
      if (!interestNodes.has(interestId)) {
        interestNodes.set(interestId, {
          count: 1,
          data: edge.interest,
        });
      } else {
        interestNodes.get(interestId)!.count++;
      }

      graphEdges.push({
        data: {
          id: `edge-${edge.user.id}-${edge.interest.id}`,
          source: userId,
          target: interestId,
        },
      });
    ...
    const interestElements = Array.from(interestNodes.entries()).map(
      ([id, { count, data: interestData }]) => {
        const size = Math.max(40, Math.min(80, 30 + count * 10));
        return {
          data: {
            id,
            label: interestData.title,
            type: "interest",
            size,
            count,
            raw: interestData,
  ...
  const graphData = useMemo(() => {
    ...
    edges.forEach((edge) => {
      ...
      const iid = edge.interest.id;

      if (!userInterests.has(uid)) userInterests.set(uid, []);
      if (!userInterests.get(uid)!.includes(edge.interest.title)) {
        userInterests.get(uid)!.push(edge.interest.title);
      }

      if (!interestUsers.has(iid)) interestUsers.set(iid, []);
      if (!interestUsers.get(iid)!.find((u) => u.id === uid)) {
        interestUsers.get(iid)!.push(edge.user);
      }
    });

    return { userInterests, interestUsers };
  }, [data]);

  const handleNodeSelect = (nodeData: any) => {
    if (nodeData.type === "user") {
      const rawUser = nodeData.raw as InterestGraphUser;
      const connectedInterests = graphData.userInterests.get(rawUser.id) || [];
      setSelectedNode({ type: "user", data: rawUser, connectedInterests });
    } else if (nodeData.type === "interest") {
      const rawInterest = nodeData.raw as InterestGraphInterest;
      const connectedUsers = graphData.interestUsers.get(rawInterest.id) || [];
      setSelectedNode({ type: "interest", data: rawInterest, connectedUsers });
    }
  };
