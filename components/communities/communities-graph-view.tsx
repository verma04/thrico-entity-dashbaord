...
type SelectedNodeInfo =
  ...
  | {
      type: "community";
      data: CommunitiesGraphCommunity;
...
export function CommunitiesGraphView() {
  ...
  const elements = useMemo(() => {
    const edges = data?.getCommunityUsersGraph || [];
    if (edges.length === 0) return [];

    const userNodes = new Map<string, any>();
    const communityNodes = new Map<string, { count: number; data: any }>();
    const graphEdges: any[] = [];

    edges.forEach((edge) => {
      const userId = `user-${edge.user.id}`;
      const communityId = `community-${edge.community.id}`;
      ...
      if (!communityNodes.has(communityId)) {
        ...
        communityNodes.get(communityId)!.count++;
      }

      graphEdges.push({
        data: {
          id: `edge-${edge.user.id}-${edge.community.id}`,
          source: userId,
          target: communityId,
        },
      });
    ...
    const communityElements = Array.from(communityNodes.entries()).map(
      ([id, { count, data: communityData }]) => {
        const size = Math.max(40, Math.min(80, 30 + count * 10));
        return {
          data: {
            id,
            label: communityData.title,
            type: "community",
            size,
            count,
            raw: communityData,
  ...
  const graphData = useMemo(() => {
    ...
    return { userCommunities, communityUsers };
  }, [data]);
  ...
  const handleNodeSelect = (nodeData: any) => {
    if (nodeData.type === "user") {
      ...
      const connectedCommunities =
        graphData.userCommunities.get(rawUser.id) || [];
      setSelectedNode({ type: "user", data: rawUser, connectedCommunities });
    } else if (nodeData.type === "community") {
      const rawCommunity = nodeData.raw as CommunitiesGraphCommunity;
      const connectedUsers =
        graphData.communityUsers.get(rawCommunity.id) || [];
      setSelectedNode({
        type: "community",
        data: rawCommunity,
        connectedUsers,
  ...
  const legend = (
    <>
      ...
      <div className="flex items-center gap-2">
        <div className="h-3 w-3 rounded bg-indigo-100 border border-indigo-200" />
        <span className="text-[10px] font-medium text-slate-500 uppercase tracking-wider">
          Communities
        </span>
      </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <EcosystemGraphView
        ...
        emptyDescription="There are no user-community relationships to visualize yet."
        legend={legend}
