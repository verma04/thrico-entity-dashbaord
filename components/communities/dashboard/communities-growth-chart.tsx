...
interface CommunitiesGrowthChartProps {
  loading: boolean;
...
export function CommunitiesGrowthChart({
  loading,
  enrollmentTrend,
  filterKey,
  onFilterChange,
  growthPercentage = 0,
  totalEnrollments,
}: CommunitiesGrowthChartProps) {
  const totalRecorded = useMemo(() => {
    if (totalEnrollments !== undefined) return totalEnrollments;
    return enrollmentTrend.reduce((acc, item) => acc + (item.count || 0), 0);
  }, [totalEnrollments, enrollmentTrend]);

  return (
    <section className="space-y-3 flex flex-col h-full">
      <DashboardSectionHeading
        title="Membership Growth & Activity"
        ...
        <CardContent className="flex-1 pb-4 pt-2 px-2 sm:px-6 relative min-h-[260px] flex flex-col justify-between">
          {loading ? (
            ...
            <div className="h-[240px] w-full flex flex-col items-center justify-center text-center text-muted-foreground">
              ...
              <span className="text-xs">No enrollment activity recorded for this period</span>
            </div>
          ) : (
