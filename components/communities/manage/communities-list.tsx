...
export function CommunitiesList({ data }: CommunitiesListProps) {
      ...
      cell: (row) => (
        <div className="flex items-center gap-3">
          <Link href={`/communities/${row.id}/about`} className="shrink-0">
            <Avatar className="h-9 w-9 rounded-lg border border-border/60 shrink-0 hover:border-primary transition-colors">
              <AvatarImage
          ...
          <div className="flex flex-col min-w-0">
            <Link
              ...
              className="text-[13px] font-semibold text-foreground leading-tight truncate hover:text-primary hover:underline transition-colors"
            >
              {row.title}
            </Link>
