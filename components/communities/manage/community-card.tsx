...
import { getStatusTag, getVerificationTag } from "../../forums/utils";

export interface CommunityCardProps {
  record: communityEntity;
}

export function CommunityCard({ record }: CommunityCardProps) {
      ...
      <Card className="overflow-hidden rounded-3xl border-slate-200/60 bg-white shadow-lg shadow-slate-200/40 hover:shadow-2xl hover:shadow-indigo-500/10 transition-all duration-500 group">
        ...
        <CardHeader className="p-5 pb-2">
          <div className="space-y-1.5">
            <div className="flex items-center gap-2 flex-wrap">
              <Link href={`/communities/${record.id}/about`}>
                <h3 className="font-black text-lg text-slate-900 tracking-tight leading-none group-hover:text-indigo-600 transition-colors hover:underline cursor-pointer">
                  {record.title}
                </h3>
              </Link>
        ...
        <CardContent className="p-5 pt-0">
          <p className="text-sm text-slate-500 font-medium line-clamp-2 min-h-[40px] leading-relaxed">
            {record.description ||
              "Inspiration, collaboration, and shared growth within our vibrant network."}
          </p>
          ...
          <div className="mt-4 flex items-center justify-between text-[10px] font-black uppercase tracking-tighter text-slate-400 px-1">
            <div className="flex items-center gap-1.5">
              ...
              {moment(record.createdAt).format("MMM YYYY")}
            </div>
            <div className="flex items-center gap-1.5">
        ...
        <CardFooter className="flex items-center justify-between border-t border-slate-50 p-5 py-4 bg-slate-50/30">
          ...
          <Button
            ...
            variant="ghost"
            size="sm"
            className="h-8 rounded-lg text-xs font-bold text-indigo-600 hover:text-indigo-700 hover:bg-indigo-50 cursor-pointer"
          >
            <Link href={`/communities/${record.id}/about`}>
              Open Space{" "}
              <ArrowUpRight className="ml-1.5 h-3.5 w-3.5 stroke-[3px]" />
            </Link>
          </Button>
        </CardFooter>
      </Card>
