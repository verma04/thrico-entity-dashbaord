...
export const Requests = () => {
  ...
  const handleApproveRequest = (_id: string) => {
    // approve logic placeholder
  };

  const handleRejectRequest = (_id: string) => {
  ...
  return (
    <Card
      loading={loading}
      title={`Join Requests (${data?.getCommunityRequest?.length || 0})`}
    >
      {!data?.getCommunityRequest || data?.getCommunityRequest?.length === 0 ? (
