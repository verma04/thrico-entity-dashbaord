...
export function ConfigPanel({ node, onUpdate, onClose, module }: ConfigPanelProps) {
  ...
  const inp = "w-full bg-white border border-slate-200 rounded-xl text-[12px] text-slate-700 px-3 py-2.5 focus:outline-none focus:border-[#5B6CFF]/60 placeholder-slate-400 transition-colors";
  ...
  return (
    <motion.div
      ...
      className="w-[320px] border-l border-slate-200 bg-white flex flex-col overflow-hidden shrink-0"
    >
      {/* Header */}
      <div className={cn("flex items-center gap-3 px-4 py-4 border-b border-slate-200", s.headerBg)}>
        ...
        <div className="flex-1 min-w-0">
          <p className="text-[13px] font-semibold text-slate-800 truncate">{node.label}</p>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-5">
        ...
        {node.type === "trigger" && (
          <div className="space-y-3">
            <label className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Trigger Event</label>
            ...
            <label className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider block">Filter by Source</label>
            <select className={inp} value={config.source || ""} onChange={(e) => setConfig({ ...config, source: e.target.value })}>
              <option value="">All (any source)</option>
              <option value="tech">Tech Communities</option>
              <option value="design">Design Communities</option>
              <option value="marketing">Marketing Communities</option>
              <option value="business">Business Communities</option>
            </select>
            <label className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider block">Note / Label</label>
        ...
        {node.type === "condition" && (
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <label className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Rule Builder</label>
              {module ? (
        ...
        {node.type === "action" && (
          <div className="space-y-3">
            <label className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Configure Action</label>

            {node.blockKey === "send-email-action" && (
              <>
                <label className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider block">Email Template</label>
                <TemplatePicker
                ...
                />
                <label className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider block mt-1">Subject Line</label>
                <input
                ...
                />
                <label className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider block">From Name</label>
            ...
            {node.blockKey === "send-notification" && (
              <>
                <label className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider block">Notification Title</label>
                <input placeholder="Title…" className={inp} value={config.title || ""} onChange={(e) => setConfig({ ...config, title: e.target.value })} />
                <label className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider block">Message</label>
            ...
            {node.blockKey === "add-tag" && (
              <>
                <label className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider block">Tag Name</label>
                ...
                <p className="text-[11px] text-slate-400">This tag will be added to the matched user's profile.</p>
              </>
            )}
            ...
            {node.blockKey === "add-to-community" && (
              <>
                ...
                <label className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider block">Select Community</label>
            ...
            {node.blockKey === "recommend-job" && (
              <>
                ...
                <label className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider block">Job Filter</label>
                ...
                <label className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider block">Delivery Method</label>
            ...
            {node.blockKey === "recommend-event" && (
              <>
                ...
                <label className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider block">Event Category</label>
            ...
            <label className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider block">Note</label>
            <input placeholder="Add a note…" className={inp} value={config.description || ""} onChange={(e) => setConfig({ ...config, description: e.target.value })} />
          </div>
        )}
        ...
        {node.type === "delay" && (
          <div className="space-y-3">
            <label className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Wait Duration</label>
            <div className="flex gap-2">
              <input type="number" min={1}
                className="flex-1 bg-white border border-slate-200 rounded-xl text-[12px] text-slate-700 px-3 py-2.5 focus:outline-none focus:border-[#5B6CFF]/60"
                value={config.amount || 1}
                onChange={(e) => setConfig({ ...config, amount: e.target.value })} />
              <select
                className="bg-white border border-slate-200 rounded-xl text-[12px] text-slate-700 px-3 py-2.5 focus:outline-none focus:border-[#5B6CFF]/60"
            ...
            <label className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider block">Delay Type</label>
      ...
      <div className="p-4 border-t border-slate-100 space-y-2">
        <div className="flex items-center gap-2 px-3 py-2.5 rounded-xl bg-indigo-50 border border-indigo-100">
          ...
          <p className="text-[12px] text-slate-600 flex-1">
            <span className="font-bold text-slate-800">2,340 users</span> match this workflow
          </p>
