...
import { Globe, ShieldCheck, CheckCircle2, AlertTriangle, ArrowRight, Server, KeyRound, FileCheck } from "lucide-react";
...
export function EmailDomainHealth({ domain }: EmailDomainHealthProps) {
        ...
        <CardContent className="p-5">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-border/40">
            ...
            <Button
              ...
              variant="outline"
              onClick={() => router.push("/settings/domains")}
              className="h-8 text-[11.5px] font-semibold rounded-[4px] border-border shrink-0"
            >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 pt-4">
            {dnsPills.map((p, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="flex items-center gap-2.5 p-2.5 rounded-[6px] bg-muted/30 border border-border/40"
                >
