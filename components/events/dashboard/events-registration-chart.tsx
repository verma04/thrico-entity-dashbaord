...
interface EventsRegistrationChartProps {
  loading: boolean;
...
export function EventsRegistrationChart({
  loading,
  registrationTrend,
  filterKey,
  onFilterChange,
  growthPercentage = 0,
  totalAttendees,
}: EventsRegistrationChartProps) {
  const totalRegistrations = useMemo(() => {
    if (totalAttendees !== undefined) return totalAttendees;
    return registrationTrend.reduce(
      (acc, item) => acc + (item.registrations || 0),
      0
    );
  }, [totalAttendees, registrationTrend]);

  return (
    <section className="space-y-3 flex flex-col h-full">
      <DashboardSectionHeading
        title="Registration Velocity & Timeline"
        ...
        <CardContent className="flex-1 pb-4 pt-2 px-2 sm:px-6 relative min-h-[260px] flex flex-col justify-between">
          {loading ? (
            ...
            <div className="h-[240px] w-full flex flex-col items-center justify-center text-center text-muted-foreground">
              ...
              <span className="text-xs">No registration data recorded for this period</span>
            </div>
          ) : (
