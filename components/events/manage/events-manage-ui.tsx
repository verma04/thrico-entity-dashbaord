...
export type EventStatusValue = (typeof STATUS_TABS)[number]["value"];
...
export const SORT_OPTIONS = [
  ...
  { value: "upcoming", label: "Upcoming Date" },
...
export function SectionHeader({
  ...
  if (status === EventStatus.ALL) return null;
          ...
          {view === "grid" ? (
            ...
            <div className="overflow-hidden rounded-xl border border-border bg-card">
              <div className="h-10 border-b border-border bg-muted/30 px-5 flex items-center gap-4">
                {[40, 160, 80, 110, 110, 70, 60, 60].map((w, i) => (
                  <Skeleton
                    key={i}
              ...
              {Array.from({ length: 8 }).map((_, i) => (
                ...
                >
                  ...
                  <Skeleton className="h-3 w-12 rounded hidden lg:block" />
                </div>
              ))}
            </div>
          )}
