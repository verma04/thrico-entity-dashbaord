    ...
    <>
      ...
      <TooltipProvider delayDuration={0}>
        <div className="flex items-center gap-3 p-3 bg-white border border-zinc-100 rounded-[28px] shadow-sm hover:shadow-md transition-all duration-300">
          ...
          >
            <UserAvatar size={34} src={data?.getEntity?.logo} className="rounded-xl shadow-sm border-2 border-white ring-1 ring-zinc-100" />
            <span className="text-[14px] font-medium text-zinc-400 group-hover:text-zinc-700 transition-colors">Start a post, or share a poll...</span>
          </div>
          ...
          <div className="flex items-center gap-1.5 pr-1">
            <Tooltip>
              ...
              <TooltipContent side="top" className="bg-zinc-900 border-none font-bold text-[11px]">Attach Media</TooltipContent>
            </Tooltip>
            ...
            <Tooltip>
              ...
              <TooltipContent side="top" className="bg-zinc-900 border-none font-bold text-[11px]">Create Poll</TooltipContent>
            </Tooltip>
          </div>
        </div>
      </TooltipProvider>
                ...
                <div className="lg:col-span-12 space-y-8">
                  ...
                  <Card className="border-none shadow-[0_4px_24px_-4px_rgba(0,0,0,0.06)] rounded-[32px] overflow-hidden bg-white ring-1 ring-zinc-100">
                    <CardHeader className="bg-zinc-50/50 pb-5 border-b border-zinc-100/50">
                      <div className="flex items-center gap-4">
                        <UserAvatar size={48} src={data?.getEntity?.logo} className="rounded-2xl border border-white shadow-md" />
                        <div>
                      ...
                      <div className="flex items-center justify-between">
                              ...
                              <TooltipTrigger asChild>
                                <Button 
                                  ...
                                  className={cn(
                                    ...
                                    (!showPoll && !pollId) ? "text-blue-500 hover:bg-blue-50" : "text-zinc-400 opacity-50 cursor-not-allowed"
                                  )}
                                  disabled={showPoll || !!pollId}
                                >
                                  <ImageIcon className="h-4.5 w-4.5" strokeWidth={2.5} />
                                  Photo/Video
                                </Button>
                              </TooltipTrigger>
                              <TooltipContent side="bottom" className="bg-zinc-900 border-none font-bold text-[11px]">Attach Images/Video</TooltipContent>
                            </Tooltip>
                            ...
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <Button 
                                  ...
                                  className={cn(
                                    "rounded-full px-5 h-10 gap-2.5 font-black text-[13px] transition-all",
                                    showPoll ? "bg-emerald-500 text-white hover:bg-emerald-600" : (fileList.length > 0 ? "text-zinc-400 opacity-50 cursor-not-allowed" : "text-emerald-500 hover:bg-emerald-50")
                                  )}
                                  onClick={() => setShowPoll(!showPoll)}
                                  disabled={fileList.length > 0}
                                >
                                  <BarChart2 className="h-4.5 w-4.5" strokeWidth={2.5} />
                                  {showPoll ? "Post with text only" : pollId ? "Poll Attached" : "Add Poll"}
                                </Button>
                              </TooltipTrigger>
                              <TooltipContent side="bottom" className="bg-zinc-900 border-none font-bold text-[11px]">Toggle Poll</TooltipContent>
                            </Tooltip>
                          </div>
                        </TooltipProvider>
                        ...
                        <div className="flex items-center gap-2">
                          <span className={cn(
                            ...
                            postText.length > 900 ? "text-orange-500" : "text-zinc-300"
                          )}>
                    ...
                    <div className="animate-in fade-in slide-in-from-top-4 duration-500">
                      <Card className="border-none shadow-[0_4px_24px_-4px_rgba(0,0,0,0.06)] rounded-[32px] overflow-hidden bg-white ring-1 ring-zinc-100">
                        <CardHeader className="bg-emerald-50/50 pb-5 border-b border-emerald-100/30 px-8">
                          <CardTitle className="text-[14px] font-black uppercase tracking-[0.15em] text-emerald-700">Create Poll</CardTitle>
                          <CardDescription className="text-[12px] font-medium text-emerald-600/70">Design an interactive poll for your post</CardDescription>
                        </CardHeader>
                    ...
                    <Card className="border-none shadow-[0_4px_24px_-4px_rgba(0,0,0,0.06)] rounded-[32px] overflow-hidden bg-emerald-50/30 border border-emerald-100 ring-1 ring-emerald-100/50">
                       <CardContent className="p-6 flex items-center justify-between px-8">
                          <div className="flex items-center gap-4">
                             ...
                             <div>
                                <p className="text-[15px] font-bold text-emerald-900 tracking-tight">Poll successfully attached!</p>
                                <p className="text-[12px] text-emerald-600/70 font-medium">Your poll will be published with this post.</p>
                             </div>
                          </div>
                          <Button 
                             variant="ghost" 
                  ...
                  {!showPoll && !pollId && (
                    ...
                    <CardHeader className="bg-zinc-50/50 pb-5 border-b border-zinc-100/50 flex flex-row items-center justify-between px-8">
                      <div>
                        <CardTitle className="text-[14px] font-black uppercase tracking-[0.15em] text-zinc-900">Attachments</CardTitle>
                        <CardDescription className="text-[12px] font-medium text-zinc-400 mt-1">Add visuals to increase high engagement</CardDescription>
                      </div>
                    ...
                    <CardContent className="pt-8 px-8 pb-8">
                      ...
                      {(!fileList || fileList.length === 0) && (
                        <div className="py-14 flex flex-col items-center justify-center border-2 border-dashed border-zinc-100 rounded-[28px] bg-zinc-50/30">
                          ...
                          <p className="text-[14px] font-bold text-zinc-400 tracking-tight">Your media will appear here</p>
                        </div>
                      )}
                    </CardContent>
                  </Card>
