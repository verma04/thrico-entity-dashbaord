...
interface FeedVelocityChartProps {
  loading: boolean;
...
export function FeedVelocityChart({
  loading,
  timelineData,
  filterKey,
  onFilterChange,
  growthPercentage = 12.4,
}: FeedVelocityChartProps) {
  const totalInteractions = useMemo(() => {
    return timelineData.reduce(
      (acc, item) => acc + (item.interactions || 0),
      0
    );
  }, [timelineData]);
  ...
  return (
    <section className="space-y-3 flex flex-col h-full">
      <DashboardSectionHeading
        title="Engagement Velocity & Timeline"
        ...
        <CardContent className="flex-1 pb-4 pt-2 px-2 sm:px-6 relative min-h-[260px] flex flex-col justify-between">
          {loading ? (
            ...
            <div className="h-[240px] w-full flex flex-col items-center justify-center text-center text-muted-foreground">
              ...
              <span className="text-xs">No feed timeline data for this period</span>
            </div>
          ) : (
            <div className="h-[240px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  <defs>
                    <linearGradient id="fillFeedInteractions" x1="0" y1="0" x2="0" y2="1">
