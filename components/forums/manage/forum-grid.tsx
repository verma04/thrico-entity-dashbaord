...
import { ForumCardCompact } from "./forum-card-compact";
...
export function ForumGrid({ forums, refetch }: ForumGridProps) {
      ...
      <Card className="border border-dashed border-border/70 shadow-none bg-muted/20">
        <CardContent className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-xs text-muted-foreground mt-1 max-w-sm">
            ...
            Try adjusting filters or start a new {singularName.toLowerCase()}.
          </p>
        </CardContent>
      </Card>
    );
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 2xl:grid-cols-5 gap-3.5">
      {forums.map((forum) => (
        <ForumCardCompact key={forum.id} forum={forum} refetch={refetch} />
      ))}
    </div>
  );
}
