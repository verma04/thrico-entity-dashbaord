...
import { ForumGrid } from "./forum-grid";
...
export type ForumStatusValue = (typeof STATUS_TABS)[number]["value"];
  ...
  return (
    <AnimatePresence mode="wait">
      {loading ? (
          ...
          {view === "grid" ? (
            ...
            <div className="overflow-hidden rounded-xl border border-border bg-card">
              <div className="h-10 border-b border-border bg-muted/30 px-5 flex items-center gap-4">
                {[40, 160, 80, 80, 70, 60, 100].map((w, i) => (
                  <Skeleton
                    key={i}
        ...
        >
          {view === "grid" ? (
            <ForumGrid forums={forums} refetch={refetch} />
          ) : (
            <ForumsList
              forums={forums}
              refetch={refetch}
