...
import {
  ...
  SectionHeader,
  ContentArea,
} from "./forums-manage-ui";
import { getForumTableColumns } from "./forums-list";
import { ExportForumsModal } from "./export-forums-modal";

export interface ForumsManageProps {
  status?: string;
}

export function ForumsManage({ status: initialStatus }: ForumsManageProps) {
  ...
  const setVerification = (v: string) =>
    updateParams({ verification: v === "ALL" ? null : v, page: null });
  ...
  const allForums: discussionForm[] = data?.getDiscussionForum || [];
  const totalCount = allForums.length;
  ...
  const filteredForums = useMemo(() => {
    ...
    if (verification !== "ALL") {
      ...
      list = list.filter((f) => !!f.verification?.isVerified === isVerified);
    }
  ...
  }, [allForums, status, verification, debouncedSearch, sortBy]);

  // Paginated slice for current page
  const paginatedForums = useMemo(() => {
    return filteredForums.slice(offset, offset + limit);
  }, [filteredForums, offset, limit]);
  ...
  return (
    <EcosystemWrapper className="gap-6">
      ...
      <EcosystemHeader
        ...
        description={
          loading
            ...
            : `${totalCount} total ${moduleName.toLowerCase()} in your community dialogues.`
        }
        icon={MessageSquare}
        breadcrumbs={[
      ...
      <EcosystemActionBar shadow="none">
        ...
        <EcosystemActionBar.Group>
          ...
          <EcosystemActionBar.Item>
            <Select
              value={verification}
              onValueChange={(v) => setVerification(v)}
            >
              <SelectTrigger className="w-[130px] h-8 rounded-md border-border bg-card text-xs font-medium text-foreground shadow-2xs focus:ring-1 focus:ring-ring">
        ...
        <EcosystemActionBar.Group align="right">
          ...
          <EcosystemActionBar.Status active={filteredForums.length > 0}>
            Showing {filteredForums.length} of {totalCount} {moduleName}
      ...
      <EcosystemContainer className="p-0 m-3 mt-0 border-none bg-transparent shadow-none ring-0 space-y-3">
        ...
        {/* Pagination Controls */}
        {!loading && filteredForums.length > limit && (
