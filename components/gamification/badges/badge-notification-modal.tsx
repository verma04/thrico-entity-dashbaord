...
import { Bell, Mail, Loader2, Award, Sparkles } from "lucide-react";
import { Badge, useUpdateBadgeNotifications } from "@/graphql/actions";
...
export function BadgeNotificationModal({
  ...
  const [updateBadgeNotifications, { loading: isUpdating }] =
    useUpdateBadgeNotifications();
  ...
  useEffect(() => {
    if (badge) {
      setAllowPush(badge.allowPushNotification !== false);
      setPushTitle(badge.pushNotificationTitle || "");
      setPushBody(badge.pushNotificationBody || "");

      setAllowEmail(badge.allowEmailNotification !== false);
      setEmailSubject(badge.emailNotificationSubject || "");
      setEmailBody(badge.emailNotificationBody || "");
    }
  }, [badge, open]);
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await updateBadgeNotifications({
        variables: {
          id: badge.id,
          input: {
        ...
        <DialogHeader className="px-6 pt-6 pb-4 border-b border-zinc-100 dark:border-zinc-800/80 bg-zinc-50/50 dark:bg-zinc-900/50">
          <div className="flex items-center gap-3">
            {badge && (
              <BadgeIcon
                icon={badge.icon}
                className="h-10 w-10 rounded-xl bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 text-xl shadow-xs shrink-0 flex items-center justify-center"
              />
            )}
            <div className="flex-1 min-w-0">
              ...
              <DialogDescription className="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5 truncate">
                Customize alert messages sent to users for{" "}
              ...
              {allowPush && (
                <div className="space-y-3.5 animate-in fade-in-50 duration-200">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Textarea
                      id="modalPushBody"
                      placeholder="e.g. Congratulations! You've earned the {{badgeName}} badge."
                      value={pushBody}
                  ...
                  <div className="flex items-center gap-1.5 text-[11px] text-zinc-500 dark:text-zinc-400">
                    ...
                    <button
                      type="button"
                      onClick={() => insertVariable("pushBody", "{{badgeName}}")}
            ...
            <TabsContent value="email" className="space-y-4 pt-3 mt-0">
              <div className="flex items-center justify-between p-3.5 rounded-xl border border-zinc-200 dark:border-zinc-700/80 bg-zinc-50/50 dark:bg-zinc-800/30">
                <div className="space-y-0.5">
                  ...
                  <p className="text-[11px] text-zinc-500 dark:text-zinc-400">
                    Send a congratulatory email when unlocked.
                  </p>
                </div>
                <Switch
                  id="allowEmailToggle"
              ...
              {allowEmail && (
                <div className="space-y-3.5 animate-in fade-in-50 duration-200">
                  ...
                  <div className="flex items-center gap-1.5 text-[11px] text-zinc-500 dark:text-zinc-400">
                    ...
                    <button
                      type="button"
                      onClick={() => insertVariable("emailBody", "{{badgeName}}")}
                      className="px-2 py-0.5 rounded-md bg-zinc-100 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 hover:bg-zinc-200 dark:hover:bg-zinc-700 border border-zinc-200/80 dark:border-zinc-700 transition-colors font-mono text-[10px]"
                    >
                      {"{{badgeName}}"}
                    </button>
