...
import { useGetLeaderboard } from "@/graphql/actions/gamification/gamification-quiries";

export function GamificationLeaderboardWidget() {
  ...
  const entries = data?.getLeaderboard?.entries || [];
  const rankMedals = ["🥇", "🥈", "🥉"];
      ...
      <CardHeader className="flex flex-row items-center justify-between pb-2 border-b border-border/40 px-3 sm:px-5 pt-3 sm:pt-4">
        <div className="space-y-0.5">
          <span className="text-[11px] font-bold text-foreground flex items-center gap-1">
            ...
            Top Community Rankers
          </span>
          <p className="text-[10px] text-muted-foreground">
            Current leaderboard standings across all tiers
          </p>
        </div>
        ...
        <Link href="/gamification/points-and-badges/leaderboard">
          ...
          >
            Full board <ArrowRight className="h-2.5 w-2.5 ml-0.5" />
          </Button>
        </Link>
      </CardHeader>
      ...
      <CardContent className="flex-1 p-0 divide-y divide-border/50">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-8 text-muted-foreground">
            ...
            <span className="text-[11px] font-medium">No leaderboard entries yet</span>
          </div>
        ) : (
          entries.map((entry) => {
            ...
            return (
              ...
              >
                ...
                <div className="text-right shrink-0">
                  ...
                  <span className="text-[9px] text-muted-foreground/70 font-medium block">
                    {entry.badgesCount || 0} badges
                  </span>
                </div>
              </div>
            );
