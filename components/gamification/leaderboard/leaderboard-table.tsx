...
export function LeaderboardTable({
  ...
  const getRankIcon = (rank: number) => {
    switch (rank) {
      ...
      case 2:
        return <Medal className="h-7 w-7 text-slate-400 drop-shadow-[0_2px_4px_rgba(148,163,184,0.4)]" />;
      case 3:
        return <Medal className="h-7 w-7 text-amber-600 drop-shadow-[0_2px_4px_rgba(180,83,9,0.4)]" />;
      default:
        return (
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (entry: LeaderboardEntry) => (
        <div className="flex items-center gap-3">
          ...
          >
            <span className="text-lg" role="img" aria-label={entry?.currentRank?.name}>
          ...
          <div className="flex flex-col">
            ...
            <div className="flex items-center gap-1">
               ...
               <span className="text-[9px] text-muted-foreground font-bold uppercase tracking-tighter">Elite Circuit</span>
            </div>
          </div>
        </div>
      ),
    ...
    {
      key: "points",
      header: "Accumulated Value",
      cell: (entry: LeaderboardEntry) => (
        <div className="flex flex-col items-end pr-4">
           <span className="font-mono text-[14px] font-black text-foreground tracking-tight">
             {entry?.totalPoints.toLocaleString()}
           </span>
           <span className="text-[9px] font-black text-muted-foreground uppercase tracking-widest">Points</span>
        </div>
      ),
    },
  ];
