...
import { RanksTableList } from "./ranks-table-list";
...
export type RankStatusValue = (typeof STATUS_TABS)[number]["value"];

export const SORT_OPTIONS = [
  { value: "order-asc", label: "Hierarchy (Tier 1 → N)" },
  { value: "order-desc", label: "Hierarchy (Tier N → 1)" },
  ...
  return (
    <div className="flex items-center gap-3 pb-1">
      ...
      >
        ...
        <span>{tab.label} Ranks</span>
      </div>
      <div className="h-px flex-1 bg-border" />
      {!loading && (
        <span className="text-[11px] font-medium text-muted-foreground">
          {count} {count === 1 ? "rank" : "ranks"}
        </span>
      )}
    </div>
  );
