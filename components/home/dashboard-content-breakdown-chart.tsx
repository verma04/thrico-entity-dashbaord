  ...
  return (
    <Card className="border-border/60 bg-gradient-to-b from-background to-muted/20 shadow-sm relative h-full flex flex-col">
      <CardHeader className="pb-2">
        <CardTitle className="text-primary tracking-wider flex items-center gap-2 text-base">
          <div className="p-1.5 rounded-md bg-primary/10"><PieChartIcon className="h-4 w-4 text-primary" /></div>
          Content Breakdown
        </CardTitle>
        <CardDescription>Distribution of content types created in the community</CardDescription>
      </CardHeader>
      ...
      <CardContent className="flex-1 flex flex-col relative pb-6 pt-2">
        ...
        {chartData.length === 0 && !loading ? (
          <div className="flex-1 flex flex-col items-center justify-center text-center opacity-70">
            ...
            <p className="text-xs text-muted-foreground">Check back later when members start posting.</p>
          </div>
        ) : (
        ...
        {!loading && chartData.length > 0 && (
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none pb-4 pr-16 md:pr-24 lg:pr-32 xl:pr-28">
            <div className="text-center">
              <span className="block text-2xl font-bold tracking-tighter text-foreground leading-none">{totalContent.toLocaleString()}</span>
              <span className="text-[10px] uppercase font-semibold text-muted-foreground tracking-widest mt-1">Total Items</span>
            </div>
          </div>
        )}
      </CardContent>
