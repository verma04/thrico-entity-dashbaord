...
import { ImpactMembersGrid } from "./impact-members-grid";
import { ImpactMembersList } from "./impact-members-list";
...
export type ImpactTierValue = (typeof TIER_TABS)[number]["value"];
...
/** Status section bar — appears between action bar and content when filtering by non-ALL tier */
export function SectionHeader({
  ...
  const tab = TIER_TABS.find((t) => t.value === tier) ?? TIER_TABS[0];
  const Icon = tab.icon;

  if (tier === "ALL" || tier === "all") return null;
  ...
  return (
    <div className="flex items-center gap-3 pb-1">
      ...
      >
        ...
        <span>{tab.label} Tier Members</span>
      </div>
  ...
  return (
    <AnimatePresence mode="wait">
      {loading ? (
          ...
          {view === "grid" ? (
            ...
            <div className="overflow-hidden rounded-xl border border-border bg-card">
              <div className="h-10 border-b border-border bg-muted/30 px-5 flex items-center gap-4">
                {[40, 180, 100, 140, 100, 100].map((w, i) => (
                  <Skeleton
                    key={i}
        ...
        >
          {view === "grid" ? (
            <ImpactMembersGrid users={users} offset={offset} />
          ) : (
            <ImpactMembersList
              users={users}
