...
import {
  STATUS_TABS,
  SORT_OPTIONS,
  ShopifyOrderStatusValue,
  SectionHeader,
  ContentArea,
} from "./shopify-orders-ui";
import { getShopifyOrderTableColumns } from "./shopify-orders-list";
import { ExportShopifyOrdersModal } from "./export-shopify-orders-modal";

export interface ShopifyOrdersManageProps {
  status?: string;
}

export function ShopifyOrdersManage({
  status: initialStatus,
}: ShopifyOrdersManageProps) {
  ...
  const totalCount = data?.getShopifyOrders?.total ?? rawOrders.length;
  ...
  const filteredOrders = useMemo(() => {
    ...
    if (status !== "ALL") {
      list = list.filter(
        (o) => (o.status || "PENDING").toUpperCase() === status.toUpperCase(),
      );
    }
    ...
    if (debouncedSearch) {
      ...
      list = list.filter(
        (o) =>
          o.shopifyOrderId?.toLowerCase().includes(term) ||
  ...
  }, [rawOrders, status, debouncedSearch, sortBy]);

  // Paginated slice for current page
  const paginatedOrders = useMemo(() => {
    return filteredOrders.slice(offset, offset + limit);
  }, [filteredOrders, offset, limit]);
...
export default ShopifyOrdersManage;
