...
import { ShopifyOrderGrid } from "./shopify-order-grid";
import { ShopifyOrdersList } from "./shopify-orders-list";
...
export type ShopifyOrderStatusValue = (typeof STATUS_TABS)[number]["value"];
  ...
  return (
    <AnimatePresence mode="wait">
      {loading ? (
          ...
          {view === "grid" ? (
            ...
            <div className="overflow-hidden rounded-xl border border-border bg-card">
              <div className="h-10 border-b border-border bg-muted/30 px-5 flex items-center gap-4">
                {[40, 160, 160, 100, 80, 80, 100].map((w, i) => (
                  <Skeleton
                    key={i}
        ...
        >
          {view === "grid" ? (
            <ShopifyOrderGrid orders={orders} refetch={refetch} />
          ) : (
            <ShopifyOrdersList
              orders={orders}
              refetch={refetch}
