...
interface JobsApplicationChartProps {
  loading: boolean;
  applicationsData: Array<{ name: string; applications: number }>;
  filterKey: string;
...
export function JobsApplicationChart({
  loading,
  applicationsData,
  filterKey,
  onFilterChange,
  growthPercentage = 0,
  totalApplications,
}: JobsApplicationChartProps) {
  const totalRecorded = useMemo(() => {
    if (totalApplications !== undefined) return totalApplications;
    return applicationsData.reduce(
      (acc, item) => acc + (item.applications || 0),
      0
    );
        ...
        <CardContent className="flex-1 pb-4 pt-2 px-2 sm:px-6 relative min-h-[260px] flex flex-col justify-between">
          {loading ? (
            ...
            <div className="h-[240px] w-full flex flex-col items-center justify-center text-center text-muted-foreground">
              ...
              <span className="text-xs">No application data recorded for this period</span>
            </div>
          ) : (
