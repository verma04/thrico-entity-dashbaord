...
const getStatusBadge = (status: string) => {
  ...
  return (
    <Badge variant={config.variant} className={`font-medium px-2.5 py-0.5 ${config.className}`}>
      {config.icon}
      {status}
    </Badge>
  );
...
const getVerificationBadge = (isVerified: boolean) => {
  return isVerified ? (
    <Badge variant="outline" className="border-blue-200 bg-blue-50 text-blue-700 font-medium px-2.5 py-0.5">
      ...
      <SheetContent className="w-full sm:max-w-3xl overflow-y-auto p-0 border-l shadow-2xl">
        <div className="relative">
          ...
          <div className="absolute top-0 left-0 right-0 h-40 bg-gradient-to-br from-primary/10 via-background to-muted border-b border-border/50 overflow-hidden">
            <div className="absolute inset-0 opacity-[0.03] mix-blend-overlay" style={{ backgroundImage: 'radial-gradient(circle at center, #000 1px, transparent 1px)', backgroundSize: '12px 12px' }}></div>
          </div>
          ...
          <div className="relative pt-24 px-8 pb-8">
            ...
            <div className="flex flex-wrap items-center gap-4 py-4 mb-6 border-y border-border/40">
              ...
              {job.experienceLevel && (
                <Badge variant="secondary" className="ml-auto rounded-md font-semibold tracking-wide">
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              <TabsList className="w-full justify-start border-b rounded-none h-12 bg-transparent p-0 space-x-6">
                {["description", "requirements", "responsibilities", "benefits"].map((tab) => (
                  <TabsTrigger 
                    key={tab}
                    value={tab} 
              ...
              <div className="py-6 min-h-[300px]">
                <TabsContent value="description" className="m-0 space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
                  ...
                  {job.skills && job.skills.length > 0 && (
                    <div className="space-y-4">
                      <h4 className="text-sm font-bold uppercase tracking-wider text-muted-foreground">Required Skills</h4>
                      <div className="flex flex-wrap gap-2">
                        {job.skills.map((skill: string, index: number) => (
                          <span key={index} className="px-3 py-1.5 bg-secondary/50 hover:bg-secondary border border-border/50 text-secondary-foreground text-sm font-medium rounded-lg transition-colors">
                            {skill}
                          </span>
                        ))}
                      </div>
                ...
                <TabsContent value="requirements" className="m-0 animate-in fade-in slide-in-from-bottom-2 duration-300">
                  <ul className="space-y-4">
                    {job.requirements && job.requirements.length > 0 ? job.requirements.map((item: string, index: number) => (
                      <li key={index} className="flex gap-4 p-4 rounded-xl bg-muted/20 border border-border/40 hover:bg-muted/40 transition-colors">
                ...
                <TabsContent value="responsibilities" className="m-0 animate-in fade-in slide-in-from-bottom-2 duration-300">
                  <ul className="space-y-4">
                    {job.responsibilities && job.responsibilities.length > 0 ? job.responsibilities.map((item: string, index: number) => (
                      <li key={index} className="flex gap-4 p-4 rounded-xl bg-muted/20 border border-border/40 hover:bg-muted/40 transition-colors">
                        <ChevronRight className="h-5 w-5 text-blue-500 shrink-0 mt-0.5" />
                        <span className="text-foreground/90 leading-relaxed">{item}</span>
                      </li>
                    )) : (
                      <p className="text-muted-foreground italic">No responsibilities listed.</p>
                    )}
                  </ul>
                </TabsContent>

                <TabsContent value="benefits" className="m-0 animate-in fade-in slide-in-from-bottom-2 duration-300">
                  <ul className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {job.benefits && job.benefits.length > 0 ? job.benefits.map((item: string, index: number) => (
                      <li key={index} className="flex gap-3 p-4 rounded-xl bg-amber-50/50 dark:bg-amber-950/20 border border-amber-100 dark:border-amber-900/50 hover:bg-amber-100/50 dark:hover:bg-amber-900/30 transition-colors">
            ...
            <div className="sticky bottom-0 -mx-8 px-8 py-6 bg-background/80 backdrop-blur-xl border-t flex justify-end gap-3 shadow-[0_-10px_40px_rgba(0,0,0,0.05)] dark:shadow-[0_-10px_40px_rgba(0,0,0,0.2)]">
              {job?.status === "PENDING" && (
                <>
                  <Button variant="outline" className="border-rose-200 hover:bg-rose-50 hover:text-rose-700 text-rose-600 transition-colors" onClick={() => handleAction("REJECT", job)}>
              ...
              {job?.status === "REJECTED" && (
                <Button variant="secondary" onClick={() => handleAction("REAPPROVE", job)}>
              ...
              {job?.status === "APPROVED" && (
                <>
                  ...
                  {job?.verification?.isVerified ? (
                    <Button variant="outline" className="border-rose-200 hover:bg-rose-50 hover:text-rose-700 text-rose-600 transition-colors" onClick={() => handleAction("UNVERIFY", job)}>
