...
import { Job, JobStatus } from "@/graphql/actions/jobs";
...
export type JobStatusValue = (typeof STATUS_TABS)[number]["value"];
...
export function SectionHeader({
  ...
  if (status === JobStatus.ALL) return null;
          ...
          {view === "grid" ? (
            ...
            <div className="overflow-hidden rounded-xl border border-border bg-card">
              <div className="h-10 border-b border-border bg-muted/30 px-5 flex items-center gap-4">
                {[40, 160, 80, 110, 70, 60, 60, 100].map((w, i) => (
                  <Skeleton
                    key={i}
