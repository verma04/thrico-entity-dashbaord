...
interface ListingsVelocityChartProps {
  loading: boolean;
  weeklyListingsData: Array<{ name: string; listings: number }>;
  filterKey: string;
...
export function ListingsVelocityChart({
  loading,
  weeklyListingsData,
  filterKey,
  onFilterChange,
  growthPercentage = 0,
  totalListings,
}: ListingsVelocityChartProps) {
  const totalRecorded = useMemo(() => {
    if (totalListings !== undefined) return totalListings;
    return weeklyListingsData.reduce(
      (acc, item) => acc + (item.listings || 0),
      0
    );
  }, [totalListings, weeklyListingsData]);

  return (
    <section className="space-y-3 flex flex-col h-full">
      <DashboardSectionHeading
        title="Catalog Expansion & Velocity"
        ...
        <CardContent className="flex-1 pb-4 pt-2 px-2 sm:px-6 relative min-h-[260px] flex flex-col justify-between">
          {loading ? (
            <div className="h-[240px] w-full flex items-center justify-center">
              <div className="flex flex-col items-center gap-2">
                ...
                <p className="text-[11px] font-medium text-muted-foreground uppercase tracking-wider">
                  Loading Catalog Trend...
                </p>
              </div>
            </div>
          ) : weeklyListingsData.length === 0 ? (
            <div className="h-[240px] w-full flex flex-col items-center justify-center text-center text-muted-foreground">
              ...
              <span className="text-xs">No catalog velocity data for this period</span>
            </div>
          ) : (
