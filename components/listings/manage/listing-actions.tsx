...
export function ListingActions({
  ...
  const [changeStatus, { loading: statusLoading }] = useChangeListingStatus({
    onCompleted: () => {
  ...
  const handleCopyLink = (e: React.MouseEvent) => {
    e.stopPropagation();
    const url = `${window.location.origin}/listing/${listing.id}/manage`;
    navigator.clipboard.writeText(url);
    toast.success("Link copied to clipboard", {
      description: `Copied link for "${listing.title}"`,
    });
  };
  ...
  const handleVerificationChange = async () => {
    try {
      const isVerified = !!listing.verification?.isVerified;
    ...
    <>
      <DropdownMenu>
        ...
        >
          ...
          <DropdownMenuItem
            onClick={() => router.push(`/listing/${listing.id}/audit-log`)}
            className="text-xs font-medium cursor-pointer gap-2 py-1.5"
          >
