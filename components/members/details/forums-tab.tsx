...
export function ForumsTab({ userId }: { userId: string }) {
  const [hasMore, setHasMore] = useState(true);
  const [isFetchingMore, setIsFetchingMore] = useState(false);
  const limit = 10;

  const { data, loading, error, fetchMore } = getDiscussionForum({
    variables: {
      input: {
        userId,
        offset: 0,
  ...
  const forumsData = data?.getDiscussionForum || [];
    ...
    try {
      const { data: fetchMoreResult } = await fetchMore({
        ...
        updateQuery(prev: any, { fetchMoreResult }: any) {
          ...
          if (fetchMoreResult.getDiscussionForum.length < limit) {
            setHasMore(false);
          }
          return {
            getDiscussionForum: [
  ...
  return (
    <div className="space-y-4">
      <ForumList data={forumsData} loading={loading && !data} />
      {hasMore && forumsData.length > 0 && (
