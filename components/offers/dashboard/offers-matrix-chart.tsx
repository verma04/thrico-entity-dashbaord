...
interface OffersMatrixChartProps {
  loading: boolean;
  data: MatrixItem[];
}

export function OffersMatrixChart({
  loading,
  data,
}: OffersMatrixChartProps) {
        ...
        <CardContent className="p-5 flex-1 flex flex-col justify-between">
          {loading ? (
            ...
            <div className="h-[260px] w-full flex flex-col items-center justify-center text-center text-muted-foreground">
              ...
              <span className="text-xs">No promotion matrix data recorded yet</span>
            </div>
          ) : (
            <>
              ...
              <div className="space-y-2 pt-3 border-t border-border/40">
                {data.map((item, i) => {
                  const itemColor =
                    item.color ||
                    OFFER_COLORS[i % OFFER_COLORS.length];
                  const percentage =
                    totalOffersCount > 0
                      ? Math.round((item.value / totalOffersCount) * 100)
                      : 0;
