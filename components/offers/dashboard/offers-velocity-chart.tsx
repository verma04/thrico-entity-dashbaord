...
interface OffersVelocityChartProps {
  loading: boolean;
  claimsTrendData: Array<{ name: string; claims: number; views?: number }>;
  filterKey: string;
...
export function OffersVelocityChart({
  loading,
  claimsTrendData,
  filterKey,
  onFilterChange,
  growthPercentage = 0,
  totalClaims,
}: OffersVelocityChartProps) {
  const totalRecorded = useMemo(() => {
    if (totalClaims !== undefined) return totalClaims;
    return claimsTrendData.reduce((acc, item) => acc + (item.claims || 0), 0);
  }, [totalClaims, claimsTrendData]);

  return (
    <section className="space-y-3 flex flex-col h-full">
      <DashboardSectionHeading
        title="Redemption Velocity & Timeline"
        ...
        <CardContent className="flex-1 pb-4 pt-2 px-2 sm:px-6 relative min-h-[260px] flex flex-col justify-between">
          {loading ? (
            ...
            <div className="h-[240px] w-full flex flex-col items-center justify-center text-center text-muted-foreground">
              ...
              <span className="text-xs">No claim activity recorded for this period</span>
            </div>
          ) : (
            <div className="h-[240px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  <defs>
                    <linearGradient id="fillOfferClaims" x1="0" y1="0" x2="0" y2="1">
