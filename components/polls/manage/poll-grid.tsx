...
import { PollCardCompact } from "./poll-card-compact";
...
export function PollGrid({ polls, refetch }: PollGridProps) {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 2xl:grid-cols-5 gap-3.5">
      {polls.map((poll) => (
        <PollCardCompact key={poll.id} poll={poll} refetch={refetch} />
      ))}
    </div>
  );
}
