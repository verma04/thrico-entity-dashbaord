...
import { Ticket, AlertTriangle, TrendingUp, Archive, BarChart3 } from "lucide-react";
...
export function InventoryTable({ items, isLoading }: InventoryTableProps) {
  const columns = [
    ...
    },
    {
      ...
      header: "Circulation",
      cell: (item: InventoryItem) => {
        const percentage = item.totalVouchers > 0 ? (item.redeemedCount / item.totalVouchers) * 100 : 0;
        return (
          <div className="flex flex-col gap-1.5 min-w-[120px]">
            <div className="flex items-center justify-between text-xs font-bold leading-none">
               <span className="text-foreground">{item.redeemedCount.toLocaleString()}</span>
               <span className="text-muted-foreground opacity-60 text-[10px] uppercase font-black">{Math.round(percentage)}% Used</span>
            </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <span
              className={cn(
                ...
                isLow ? "text-rose-600 underline decoration-rose-200" : "text-emerald-600",
              )}
            >
        ...
        return (
          <div className="flex items-center gap-2 pr-4">
            <div className={cn(
               ...
               expiring > 0 ? "bg-amber-50 border-amber-200 text-amber-600" : "bg-zinc-50 border-zinc-200 text-zinc-400"
            )}>
