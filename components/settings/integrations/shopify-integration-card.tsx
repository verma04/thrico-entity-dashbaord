...
import { ShoppingBag, Loader2, Users, Package, ShoppingCart } from "lucide-react";
          ...
          <div className="space-y-3">
            <div className="flex flex-col gap-1 p-2 bg-muted/50 rounded-lg border border-border/50">
              <div className="flex justify-between items-center">
                <span className="text-[10px] text-muted-foreground uppercase font-semibold">Connected Store</span>
                <span className="text-xs font-medium">{connection?.shopDomain}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-[10px] text-muted-foreground uppercase font-semibold">Last Synced</span>
            ...
            <div className="space-y-2">
              <Label className="text-[10px] font-semibold uppercase text-muted-foreground tracking-wider">Manual Sync</Label>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-[400px] p-5">
          ...
          <DialogFooter className="mt-4 gap-2 sm:gap-0">
            <CtaButton variant="outline" onClick={() => setIsDialogOpen(false)} disabled={isConnecting}>
              Cancel
            </CtaButton>
            <CtaButton onClick={submitConnect} disabled={isConnecting || !shopDomain}>
              {isConnecting ? (
                <>
