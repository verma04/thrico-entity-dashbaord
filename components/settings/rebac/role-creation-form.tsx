...
export function RoleCreationForm({
  ...
  useEffect(() => {
    if (initialValues) {
      setFormData({ name: initialValues.name, description: initialValues.description || "" });
  ...
  return (
    <div className="flex flex-col h-full bg-background overflow-hidden rounded-t-[inherit]">
        ...
        <div className="max-w-7xl mx-auto w-full flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <div className="flex items-center gap-2 text-xs text-muted-foreground ml-1">
              ...
              <span>Roles</span>
              <ChevronRight className="h-3 w-3" />
              <span>{isEditing ? "Edit Role" : "Create Role"}</span>
            </div>
          </div>
        </div>
      </div>
          ...
          <Card className="border-none shadow-sm ring-1 ring-border/50 overflow-hidden">
            <CardHeader className="bg-muted/30 pb-4">
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle className="text-xl">Admin Access Scopes</CardTitle>
                  <CardDescription>
                ...
                <label className="flex items-center gap-2 cursor-pointer bg-background border px-3 py-1.5 rounded-lg">
                  ...
                  />
                  <span className="text-xs font-medium">Select all scopes</span>
                </label>
              </div>
            </CardHeader>
          ...
          <Card className="border-none shadow-sm ring-1 ring-border/50 overflow-hidden">
            <CardHeader className="bg-muted/30 pb-4">
              <div className="flex items-center justify-between flex-wrap gap-4">
                <div>
                  <CardTitle className="text-xl">Module Permissions</CardTitle>
                  <CardDescription>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-3 min-w-[160px]">
                          ...
                          >
                            <ModuleIcon name={moduleName} className="w-4 h-4" />
                          </div>
                        ...
                        <div className="flex items-center gap-2 ml-auto">
                          {permissionTypes.map((type) => {
                            const isChecked = !!permissions[moduleName]?.[type];
                            return (
                              <button
                                key={type}
                                type="button"
                    ...
                    <CardContent>
                      <ul className="space-y-3 text-sm">
                        ...
                        <li className="flex gap-2">
                          <span className="text-primary font-bold">•</span>
                          <span>
                            Follow the principle of least privilege — only
      ...
      <FloatingSavePanel
        ...
        onSave={handleSubmit}
        onReset={handleReset}
        title={isEditing ? "Unsaved Role Updates" : "Unsaved Role"}
        description="You have unfilled form data or unsaved changes."
        buttonText={isEditing ? "Save Changes" : "Create Role"}
        disableSubmit={!formData.name.trim() || loading}
      />
    </div>
  );
}
