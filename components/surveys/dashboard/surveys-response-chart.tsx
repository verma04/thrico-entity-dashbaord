...
interface SurveysResponseChartProps {
  loading: boolean;
  responseTrendData: Array<{ name: string; responses: number }>;
  filterKey: string;
...
export function SurveysResponseChart({
  loading,
  responseTrendData,
  filterKey,
  onFilterChange,
  growthPercentage = 0,
  totalResponses,
}: SurveysResponseChartProps) {
  const totalRecorded = useMemo(() => {
    if (totalResponses !== undefined) return totalResponses;
    return responseTrendData.reduce(
      (acc, item) => acc + (item.responses || 0),
      0
    );
  }, [totalResponses, responseTrendData]);

  return (
    <section className="space-y-3 flex flex-col h-full">
      <DashboardSectionHeading
        title="Response Velocity & Timeline"
        ...
        <CardContent className="flex-1 pb-4 pt-2 px-2 sm:px-6 relative min-h-[260px] flex flex-col justify-between">
          {loading ? (
            ...
            <div className="h-[240px] w-full flex flex-col items-center justify-center text-center text-muted-foreground">
              ...
              <span className="text-xs">No response data recorded for this period</span>
            </div>
          ) : (
            <div className="h-[240px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  <defs>
                    <linearGradient id="fillSurveyResponses" x1="0" y1="0" x2="0" y2="1">
