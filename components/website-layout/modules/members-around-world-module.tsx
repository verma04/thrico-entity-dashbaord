...
export const MembersAroundWorldModule = ({
  ...
  const continents = [...new Set(locations.map((loc: any) => loc.continent))].filter(Boolean);
  ...
  return (
    <ModuleContainer containerSettings={content.containerSettings} className="bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50">
      <div className="text-center mb-12">
            ...
            <div className="bg-white border-2 rounded-2xl p-8 min-h-[500px] relative overflow-hidden shadow-xl">
              ...
              <div className="relative">
                <div className="text-center mb-8">
                  ...
                  <h3 className="text-2xl font-bold mb-2">Global Member Distribution</h3>
                  <p className="text-muted-foreground">
                    Heatmap showing {totalMembers.toLocaleString()} members across {totalCountries} countries
                  </p>
                </div>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8">
                  {locations.map((location: any, idx: number) => {
                    const intensity = (location.memberCount || 0) / Math.max(...locations.map((l: any) => l.memberCount || 1));
                    const Content = (
                      <div
        ...
        {layout === "country-stats-grid" && locations.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {locations.map((location: any, idx: number) => {
              const Content = (
                 <div
                  className="bg-white border-2 rounded-xl overflow-hidden hover:shadow-2xl transition-all hover:scale-105 group h-full"
                >
          ...
          <div className="space-y-8">
            <div className="bg-gradient-to-br from-indigo-900 via-purple-900 to-pink-900 rounded-3xl p-12 min-h-[600px] relative overflow-hidden shadow-2xl">
              ...
              <div className="relative text-center">
                ...
                <h3 className="text-4xl font-bold text-white mb-4">Interactive 3D Globe</h3>
                <p className="text-white/80 text-lg mb-8">
                  Explore {totalMembers.toLocaleString()} members across {totalCountries} countries
                </p>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-4xl mx-auto">
                  {locations.slice(0, 8).map((location: any, idx: number) => {
                    const Content = (
                      <div
                        className="bg-white/10 backdrop-blur-md border border-white/20 rounded-xl p-4 text-center hover:bg-white/20 transition-all cursor-pointer h-full"
                      >
        ...
        {layout === "regional-cards" && locations.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {continents.map((continent: any, idx: number) => {
              const regionLocations = locations.filter((loc: any) => loc.continent === continent);
              const regionMembers = regionLocations.reduce((sum: number, loc: any) => sum + (loc.memberCount || 0), 0);
              const regionGrowth = regionLocations.reduce((sum: number, loc: any) => sum + (loc.growth || 0), 0) / regionLocations.length;
                ...
                >
                  <div className="bg-gradient-to-br from-blue-500 to-purple-600 p-8 text-white">
                    <div className="text-6xl mb-4">
                      {continent === "Asia" ? "🌏" : continent === "Europe" ? "🌍" : continent === "Africa" ? "🌍" : continent === "North America" ? "🌎" : continent === "South America" ? "🌎" : "🌏"}
                    </div>
                    <h3 className="text-3xl font-bold mb-2">{continent || `Region ${idx + 1}`}</h3>
                    <p className="text-white/80">{regionLocations.length} countries</p>
                  </div>
                  <div className="p-6">
                    ...
                    <div className="space-y-2">
                      {regionLocations.slice(0, 3).map((loc: any, locIdx: number) => {
                        const Item = (
                        ...
                        return (
                          <div key={locIdx} className="flex items-center justify-between text-sm">
                            {loc.link ? (
                              <a href={loc.link} target="_blank" rel="noopener noreferrer" className="hover:underline flex items-center gap-2">
                                {Item}
                              </a>
                            ) : (
                              Item
                            )}
                            <span className="font-semibold">{loc.memberCount || 0}</span>
                          </div>
                        );
                      })}
                    </div>
            ...
            <div className="bg-white border-2 rounded-2xl p-8 min-h-[500px] relative overflow-hidden shadow-xl">
              ...
              <div className="relative">
                <div className="text-center mb-8">
                  ...
                  <h3 className="text-2xl font-bold mb-2">Member Locations Worldwide</h3>
                  <p className="text-muted-foreground">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                  {locations.map((location: any, idx: number) => {
                    const Content = (
                      <div
                        className="bg-gradient-to-br from-red-50 to-pink-50 border-2 border-red-200 rounded-xl p-4 text-center hover:shadow-lg transition-all hover:scale-105 relative h-full"
                      >
        ...
        {layout === "member-density-chart" && locations.length > 0 && (
          <div className="bg-white border-2 rounded-2xl p-8 shadow-xl">
            <div className="text-center mb-8">
              <div className="text-6xl mb-4">📊</div>
              <h3 className="text-2xl font-bold mb-2">Member Density by Country</h3>
              <p className="text-muted-foreground">Visual comparison of member distribution</p>
            </div>
            <div className="space-y-4 max-w-4xl mx-auto">
              {sortedLocations.map((location: any, idx: number) => {
                ...
                return location.link ? (
                  <a key={idx} href={location.link} target="_blank" rel="noopener noreferrer" className="block group">
                    <div className="space-y-2">
                       <div className="flex items-center justify-between group-hover:text-blue-600 transition-colors">
                        <div className="flex items-center gap-3">
                          <span className="text-2xl">{location.flag || "🌍"}</span>
                          <span className="font-semibold">{location.country || `Country ${idx + 1}`}</span>
                        </div>
                        <span className="font-bold text-lg">{location.memberCount || 0}</span>
                      </div>
            ...
            <div className="bg-gradient-to-br from-amber-50 to-orange-50 border-2 border-amber-200 rounded-2xl p-8 shadow-xl">
              ...
              <div className="space-y-4">
                {sortedLocations.map((location: any, idx: number) => {
                  const Content = (
                      <div
                        className={cn(
                          ...
                          idx === 0 && "border-amber-400 bg-gradient-to-r from-amber-50 to-yellow-50",
                          idx === 1 && "border-gray-400 bg-gradient-to-r from-gray-50 to-slate-50",
                          idx === 2 && "border-orange-400 bg-gradient-to-r from-orange-50 to-amber-50"
                        )}
                      >
                  ...
                  return location.link ? (
                    <a key={idx} href={location.link} target="_blank" rel="noopener noreferrer" className="block transition-transform hover:scale-[1.01]">
                       {Content}
                    </a>
                  ) : (
                    <div key={idx}>
          ...
          <div className="space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {continents.map((continent: any, idx: number) => {
                const regionLocations = locations.filter((loc: any) => loc.continent === continent);
                const regionMembers = regionLocations.reduce((sum: number, loc: any) => sum + (loc.memberCount || 0), 0);
                const percentage = ((regionMembers / totalMembers) * 100).toFixed(1);
                ...
                return (
                  ...
                  >
                    <div className="text-center mb-6">
                      ...
                      <h3 className="text-2xl font-bold mb-2">{continent || `Continent ${idx + 1}`}</h3>
                    </div>
                    <div className="space-y-4">
                      <div className="text-center">
                        ...
                        <div className="text-sm text-muted-foreground">of total members</div>
                      </div>
                      ...
                      <div className="flex items-center justify-between text-sm">
                        <span className="text-muted-foreground">{regionLocations.length} countries</span>
                        <span className="font-bold">{regionMembers.toLocaleString()} members</span>
                      </div>
                    </div>
                  </div>
                );
          ...
          <div className="max-w-5xl mx-auto">
            <div className="bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 rounded-2xl p-12 text-white shadow-2xl">
              <div className="flex flex-col md:flex-row items-center justify-around gap-8 text-center">
                ...
                <div className="space-y-2">
                  <div className="text-6xl font-bold">{totalMembers.toLocaleString()}+</div>
              ...
              return (
                <div key={idx} className="space-y-6">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="text-3xl font-bold">{continent || `Region ${idx + 1}`}</h3>
                      <p className="text-muted-foreground">
                        {regionLocations.reduce((sum: number, loc: any) => sum + (loc.memberCount || 0), 0)} members across {regionLocations.length} countries
                      </p>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
                    {regionLocations.map((location: any, locIdx: number) => {
                      const Content = (
                        <div
                          className="aspect-square bg-gradient-to-br from-blue-100 to-purple-100 rounded-xl overflow-hidden hover:shadow-xl transition-all hover:scale-105 relative group h-full w-full"
                        >
                          {location.image ? (
                            <img
                              src={location.image}
                          ...
                          <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-end p-3">
                            <div className="text-white text-center">
                              <div className="font-bold text-sm mb-1">{location.country}</div>
                              <div className="text-xs">{location.memberCount || 0} members</div>
                            </div>
                          </div>
                        </div>
                      );
                      ...
                      return location.link ? (
                        <a key={locIdx} href={location.link} target="_blank" rel="noopener noreferrer" className="block">
                          {Content}
                        </a>
                      ) : (
                        <div key={locIdx}>
