...
export const MilestonesModule = ({
  ...
  const getStatusInfo = (status: string) => {
    const statusMap: Record<string, { color: string; bg: string; icon: any; text: string }> = {
      completed: { color: "text-green-600", bg: "bg-green-100", icon: CheckCircle2, text: "Completed" },
      "in-progress": { color: "text-blue-600", bg: "bg-blue-100", icon: Clock, text: "In Progress" },
      upcoming: { color: "text-orange-600", bg: "bg-orange-100", icon: Calendar, text: "Upcoming" },
      planned: { color: "text-gray-600", bg: "bg-gray-100", icon: Flag, text: "Planned" },
    };
            ...
            {milestones.map((milestone: any, idx: number) => {
              ...
              return (
                <div key={idx} className="relative pl-20">
                  <div className={`absolute left-6 w-5 h-5 ${statusInfo.bg} rounded-full border-4 border-white flex items-center justify-center`}>
                  ...
                  <div className="bg-white border rounded-lg p-4 hover:shadow-md transition-shadow">
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <h3 className="font-bold text-lg">{milestone.title || `Milestone ${idx + 1}`}</h3>
                      </div>
                      <span className={`${statusInfo.bg} ${statusInfo.color} text-xs px-2 py-1 rounded-full font-medium`}>
                        {statusInfo.text}
                      </span>
                    </div>
                    ...
                    {milestone.image && (
                      <img src={milestone.image} alt={milestone.title} className="w-full h-32 object-cover rounded-lg mb-3" />
                    )}
                    {milestone.date && (
                ...
                <div key={idx} className="relative">
                  <div className="w-64 bg-white border rounded-lg p-4">
                    ...
                    <div className="flex items-start gap-2 mb-2">
                      ...
                      <h3 className="font-semibold text-sm">{milestone.title || `Milestone ${idx + 1}`}</h3>
                    </div>
                    {milestone.description && (
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className={`${statusInfo.bg} ${statusInfo.color} text-xs px-2 py-0.5 rounded-full font-medium`}>
                        {statusInfo.text}
                      </span>
                    </div>
                  </div>
      ...
      {layout === "milestone-cards" && milestones.length > 0 && (
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {milestones.map((milestone: any, idx: number) => {
            ...
            return (
              <div key={idx} className="bg-white border rounded-xl overflow-hidden hover:shadow-lg transition-shadow">
                {milestone.image ? (
                  <div className="relative h-40 overflow-hidden">
                    <img src={milestone.image} alt={milestone.title} className="w-full h-full object-cover" />
                    <div className="absolute top-3 right-3">
                      <span className={`${statusInfo.bg} ${statusInfo.color} text-xs px-3 py-1 rounded-full font-medium backdrop-blur-sm`}>
                        {statusInfo.text}
                      </span>
                    </div>
                  </div>
                ...
                <div className="p-4">
                  ...
                  {milestone.date && (
                    <div className="flex items-center gap-2 text-xs text-muted-foreground">
                      ...
                      {new Date(milestone.date).toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}
                    </div>
                  )}
                </div>
              </div>
      ...
      {layout === "achievement-list" && milestones.length > 0 && (
        <div className="space-y-4">
          {milestones.map((milestone: any, idx: number) => {
            ...
            return (
              <div key={idx} className={`flex items-start gap-4 p-4 rounded-lg border-l-4 ${statusInfo.bg} bg-opacity-50 hover:bg-opacity-100 transition-all`} style={{ borderColor: statusInfo.color.replace('text-', '') }}>
                {milestone.image ? (
                  <img src={milestone.image} alt={milestone.title} className="w-20 h-20 object-cover rounded-lg flex-shrink-0" />
                ) : milestone.icon ? (
                  <div className={`w-20 h-20 ${statusInfo.bg} rounded-lg flex items-center justify-center flex-shrink-0`}>
                    {renderIcon(milestone.icon, "w-10 h-10 " + statusInfo.color)}
                  </div>
                ) : (
                  <div className={`w-20 h-20 ${statusInfo.bg} rounded-lg flex items-center justify-center flex-shrink-0`}>
                ...
                <div className="flex-1">
                  <div className="flex items-start justify-between mb-2">
                    <h3 className="font-bold text-lg">{milestone.title || `Milestone ${idx + 1}`}</h3>
                    <span className={`${statusInfo.bg} ${statusInfo.color} text-xs px-3 py-1 rounded-full font-medium flex items-center gap-1`}>
