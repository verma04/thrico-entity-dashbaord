...
export const ModuleContainer = ({
  ...
  // Check if background is a custom color (hex, rgb, etc.) or a Tailwind class
  const isCustomColor = background && (
  ...
  const shouldUseContainerBackground = containerSettings?.background !== undefined;
  const finalBackgroundClass = shouldUseContainerBackground ? backgroundClass : '';

  // Button style classes
  const getButtonStyles = (style: string) => {
    const baseStyles = "inline-flex items-center justify-center px-6 py-3 rounded-lg font-semibold transition-all duration-200 text-sm";
    const styles = {
    ...
    return cn(baseStyles, styles[style as keyof typeof styles] || styles.primary);
  };
