...
export const PartnersModule = ({
  ...
  return (
    <ModuleContainer containerSettings={content.containerSettings} className="bg-gray-50 border-y">
      <ModuleHeader
        title={content.title}
            ...
            {partners.map((partner: any, idx: number) => (
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="font-medium">{partner.name || `Partner ${idx + 1}`}</h3>
                    <p className="text-sm text-muted-foreground">
                      {partner.category || partner.description || "Technology Partner"}
                    </p>
                  </div>
                </div>
                {partner.website && (
